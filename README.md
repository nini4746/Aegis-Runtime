...
- **알고리즘 라이프사이클**: 각 알고리즘을 프로세스처럼 ACTIVE/THROTTLED/DEAD/RECOVERING 상태로 관리. `LifecycleSampler`(주기 샘플링)가 워커 EWMA·실패율·메모리 압력 스냅샷을 순수 함수 `PolicyEngine`에 넘겨 전이 결정(임계값·kill-order·min-share는 `policy.*` config). DEAD = 신규 admission 거부 + 검증 캐시 무효화(서명 key는 보존, 기존 토큰은 fallback 직접 검증). `GET /admin/arena`로 알고리즘별 상태·score 조회. 설계·자율 결정은 `.planning/lifecycle-SPEC.md`.
...
- `GET /admin/arena` — 알고리즘 라이프사이클 Leaderboard (상태·score·마지막 kill 사유)
